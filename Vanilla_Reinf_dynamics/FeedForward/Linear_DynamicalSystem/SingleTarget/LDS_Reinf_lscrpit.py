...
# use following arguments: -c 1 -t 70 -m 10  --cmd python LSD_Reinf_sendTraining.py
...
# One arm:
actor_ln = torch.tensor([5e-05, 7.5e-05, 0.0001, 0.00025, 0.0005, 0.00075, 0.001]) #torch.linspace(1e-04,1e-02,7) #  5e-04
std_range = torch.linspace(0.01,0.015,4) # 0.0113
...
if args.autoname:
    ...
    print(f"{cmd} 2>&1 | tee {path}.o")
     ...
     # One arm:
     seeds = np.random.choice(1000,size=5) #np.random.choice([i for i in range(0, 1000) if i not in [37, 235, 908, 72, 767]], size=10)
     ...
     for seed in seeds:
        ...
        for std in std_range:
            for act in actor_ln:
                ...
                print(cmd +" -s " + str(seed) + " -d " + str(std.item()) + " -a " + str(act.item()) + " -i " + str(i),' -hs ' +str(True))
                i +=1