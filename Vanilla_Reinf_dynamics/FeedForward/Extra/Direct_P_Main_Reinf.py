from Extra_models.Direct_Cntrl_Parall_Arm_model import Direct_Pl_Arm_model
...
n_arms = 5000
tspan = [0, 0.4]
x0 = [[-np.pi / 2], [np.pi / 2], [0], [0]] # initial condition, needs this shape
...
training_arm = Direct_Pl_Arm_model(tspan,x0,dev, n_arms=n_arms)
...
for ep in range(episodes):
    ...
    thetas = training_arm.perform_reaching(t_step,actions)
    ...
    if ep % t_print == 0:
        ...
        print("mus: ", torch.norm(agent.mu_s))
        print("actions: ", torch.norm(torch.mean(actions,dim=0)))
        ep_rwd = []
        ep_vel = []
...
test_arm = Direct_Pl_Arm_model(tst_tspan,x0,dev,n_arms=1)
