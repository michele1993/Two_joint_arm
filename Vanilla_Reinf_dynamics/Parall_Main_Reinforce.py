...
for ep in range(episodes):
    ...
    weighted_adv = advantage + velocity_adv * vel_weight

    agent.update(weighted_adv)

    ep_rwds.append(rwd)
    ep_vels.append(velocity)

    if ep % t_print == 0:

        print("episode: ", ep)
        print("training accuracy: ",torch.mean(sum(ep_rwds)/t_print))
        print("training velocity: ", torch.mean(sum(ep_vels) / t_print),'\n')
        ep_rwds = []
...
torch.save(t_y, 'Results/test_dynamics_1.pt')
torch.save(test_actions, 'Results/test_actions_1.pt')

tst_accuracy = test_arm.compute_rwd(t_y,x_hat,y_hat)
tst_velocity = test_arm.compute_vel(t_y)
