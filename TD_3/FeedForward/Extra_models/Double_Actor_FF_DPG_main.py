...
th_error = 0.025
decay_upd = [0.75,0.8,0.85]#torch.linspace(0.05,0.95,21)#0.05 #0.005
...
beh_agent = Actor_NN(dev, Output_size=n_parametrised_steps * 2, ln_rate=ln_rate_a).to(dev)
beh_agent.load_state_dict(agent.state_dict())
beh_agent.freeze_params()

c_input_a = n_parametrised_steps * 2
critic_1 = Critic_NN(n_arms, dev, a_size =c_input_a, ln_rate=ln_rate_c).to(dev)
...
for d_u in decay_upd:
    ...
    for ep in range(1, episodes):
        ...
        det_actions = beh_agent(target_state)  # may need converting to numpy since it's a tensor
        ...
        Tar_Q = critic_1(target_state, agent(target_state), True)  # want to max the advantage

        # COuld try to move Q_v also towards Tar_Q to avoid over training of Q due to slow change of
        # behave agent
        ...
        if ep > start_a_upd:
            ...
            agent.update(Tar_Q)
            beh_agent.soft_update(agent,d_u)
