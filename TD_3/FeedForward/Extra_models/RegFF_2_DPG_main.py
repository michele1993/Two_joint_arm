...
# (Not working well) Implement DPG for the FeedForward arm model, inputting the desired location to a actor NN,
...
# can be from the actual return, but in this version update Actor more often than critique.
...
vel_weight = 0.8#0.6#0.8
ln_rate_c = 0.01#0.01 # 0.005 #0.001
ln_rate_a = 0.00001#0.00001 #0.00001 #0.000005
std = 0.01#0.01 #0.01 #0.0001
max_u = 15000
actor_update = 8#5 #2 #5 #4 reaches 18cm distance and stops
start_a_upd  = 1#500
Q_reg_weight = 40#50
...
for ep in range(1,episodes):
    ...
    ep_meanG.append(mean_G)
    ep_stdG.append(std_G.unsqueeze(0))
    ...
    if ep % t_print == 0:
        ...
        print("aver G", torch.mean(torch.cat(ep_meanG)))
        print("aver std G",torch.mean(torch.cat(ep_stdG)),"\n")
        ep_rwd = []
        ep_vel = []
        ep_stdG = []
        ep_meanG = []
