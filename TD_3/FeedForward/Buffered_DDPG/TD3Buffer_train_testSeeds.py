...
class TD3_train:
    ...
    def __init__(self, critic_ln,actor_ln,std,episodes,n_arms,dev):
        ...
        self.M_buffer = V_Memory_B(self.a_size, self.n_arms, dev, batch_size=self.batch_size, size=self.buffer_size)
    ...
    def train(self):
        ...
        for ep in range(1, self.episodes):
            ...
            exploration = (torch.randn((self.n_arms, self.a_size)) * self.std).to(self.dev)

            Q_actions = (det_actions.view(1, self.a_size) + exploration).detach()

            actions = Q_actions.view(-1, 2, self.n_parametrised_steps) * self.max_u
            ...
            # Store transition
            self.M_buffer.store_transition(self.target_state, Q_actions, weighted_adv)

            # Sample from buffer

            spl_t_state, spl_a, spl_rwd = self.M_buffer.sample_transition()

            Q_v = self.critic_1(spl_t_state, spl_a, True)

            self.critic_1.update(spl_rwd, Q_v)


            if ep > self.start_a_upd and ep % self.actor_upd == 0:
