...
class V_Memory_B:
    ...
    def store_transition(self,c_state,action,rwd,n_state,dn):
        ...
        self.c_state_buf[c_idx,:] = c_state.to('cpu')
        self.actions_buf[c_idx,:] = action.to('cpu')
        self.rwd_buf[c_idx,:] = rwd.to('cpu')
        self.n_state_buf[c_idx,:] = n_state.to('cpu')
        self.done_buf[c_idx,:] = dn.to('cpu')
    ...
    def sample_transition(self):
        ...
        spl_c_state = self.c_state_buf[indx,:].view(-1,self.s_space,1).to(self.dev)
        spl_a = self.actions_buf[indx,:].view(-1,self.a_space).to(self.dev)
        spl_rwd = self.rwd_buf[indx,:].view(-1,1).to(self.dev)
        spl_n_state = self.n_state_buf[indx,:].view(-1,self.s_space,1).to(self.dev)
        spl_done = self.done_buf[indx,:].view(-1,1).to(self.dev)
