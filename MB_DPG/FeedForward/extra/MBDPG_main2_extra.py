...
std = 0.01
max_u = 15000
start_a_upd = 100 # 1000 performs much worse
...
deterministic_arm = FF_Parall_Arm_model(tspan, x0, dev, n_arms=1)
...
for ep in range(1, episodes):
    ...
    actions = (det_actions.view(1, 2, -1) + exploration)
    ...
    if ep > start_a_upd: # and ep % 3 ==0:
        ...
        det_actions = agent(target_state)

        _, thetas = deterministic_arm.perform_reaching(t_step,det_actions.view(1, 2, -1))
        ...
        weight_rwd = rwd + vel * vel_weight

        # compute gradient of rwd with respect to outcome
        dr_dy = torch.autograd.grad(outputs=weight_rwd, inputs = diff_thetas)[0]

        est_y = est_arm(det_actions) # re-estimate values since model has been updated

        # compute gradient of rwd with respect to actions, using environment outcome
        dr_da = torch.autograd.grad(outputs= est_y, inputs = det_actions, grad_outputs= dr_dy.squeeze(dim = 0))[0]

        agent.MB_update(det_actions,dr_da)
