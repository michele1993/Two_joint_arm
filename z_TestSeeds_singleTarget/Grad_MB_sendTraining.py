...
parser.add_argument('--modelLr',    '-m', type=float, nargs='?',default= 5.0500e-03) # Best from Hyper-search: 5.0500e-03
parser.add_argument('--actorLr',   '-a', type=float, nargs='?', default= 8.7500e-05) # Best from Hyper-search: 8.7500e-05
...
if not search_hyperParam:
    ...
    accuracy_file = '/user/home/px19783/Two_joint_arm/Model_based/GradientBased/SingleTarget/Results/Grad_MB_training_acc_test_s'+str(seed)+'_'+str(i)+'_oneArmOptim.pt'
    actor_file = '/user/home/px19783/Two_joint_arm/Model_based/GradientBased/SingleTarget/Results/Grad_MB_actor_test_s'+str(seed)+'_'+str(i)+'_oneArmOptim.pt'
    model_file = '/user/home/px19783/Two_joint_arm/Model_based/GradientBased/SingleTarget/Results/Grad_MB_model_test_s'+str(seed)+'_'+str(i)+'_oneArmOptim.pt'
    episodes = 10001
else:

    accuracy_file = '/user/home/px19783/Two_joint_arm/Model_based/GradientBased/SingleTarget/Hyperparam_tuning/Result/Grad_MB_training_acc_hyperTuning_s' + str(
...
agent = Actor_NN(dev, Output_size=n_parametrised_steps * 2, ln_rate=actor_ln).to(dev)
...
for ep in range(1, episodes):
    ...
    if ep > start_a_upd: #and ep % actor_update ==0:
        ...
        est_y = est_arm(actions.view(n_arms,a_size)) # estimate value of undertaken action - buffer only used to update model not actor


        # Compute the estimated reward and velocity
        est_rwd = training_arm.compute_rwd(est_y.unsqueeze(0), x_hat, y_hat, f_points)
        est_vel = training_arm.compute_vel(est_y.unsqueeze(0), f_points)

        est_w_rwd = torch.sum(est_rwd + est_vel * vel_weight)

        # compute gradient of rwd with respect to outcome
        dr_dy = torch.autograd.grad(outputs=est_w_rwd, inputs = est_y)[0]


        # compute gradient of rwd with respect to actions, using environment outcome
        dr_da = torch.autograd.grad(outputs= est_y, inputs = actions, grad_outputs= dr_dy)[0]
